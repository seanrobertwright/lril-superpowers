...
> **For agentic workers:** REQUIRED SUB-SKILL: Use lril-superpowers:executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "lril-superpowers:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip lril-superpowers: prefix if present
    const forceSuperpowers = skillName.startsWith('lril-superpowers:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^lril-superpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly lril-superpowers:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "lril-superpowers:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'lril-superpowers:';
...
**Skills naming:**
- Superpowers skills: \`lril-superpowers:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'lril-superpowers:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "lril-superpowers:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill lril-superpowers:brainstorming | head -20`
